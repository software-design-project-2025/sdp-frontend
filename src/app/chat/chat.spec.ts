...
export class Chat implements OnInit, AfterViewChecked {
  ...
  private async getCurrentUserId(): Promise<string> {
    try {
      const result = await (this.authService.getCurrentUser() as any);
  ...
  private async formatConvos(userid: string): Promise<Conversation[]> {
    try {
      const result = await firstValueFrom(this.chatService.getChatById(userid) as any) as any[];
  ...
  private async retrieveGroupConvos(userid: string): Promise<GroupConversation[]> {
    try {
      const result = await firstValueFrom(this.chatService.getGroups(userid) as any) as any[];
      ...
      const groupPromises = result.map(async (group: any) => {
        ...
        members.map(async (member) => {
          try {
            const [supabaseData, postgresData] = await Promise.all([
              this.authService.getUserById(member.userid) as any,
  ...
  private async getOtherUserName(convo: Conversation): Promise<string | undefined> {
    try {
      const result = await (this.authService.getUserById(convo.participant.userid) as any);
      if (result) {
    ...
    if (this.isGroupConversation(conversation)) {
      ...
      for (let i = conversation.messages.length - 1; i >= 0; i--) {
        if (conversation.messages[i].read_status == false && conversation.messages[i].senderid != this.currentUser.userid) {
          const promise = firstValueFrom(this.chatService.updateStatus(conversation.messages[i].id, true) as any)
  ...
  private async handleFileUpload(file: File): Promise<void> {
    ...
    try {
      ...
      if (this.isGroupConversation(activeConvo)) {
        ...
        const docMessage: ExtendedGroupMessage = {
      ...
      activeConvo.timestamp = new Date(); // Use new Date() for consistency
      this.shouldScrollToBottom = true;
      this.cdr.detectChanges();
